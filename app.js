...
const changeBackground = (status) => {
  if (status === "Clear") {
    ...
    document.body.style.background = "url(./images/mist.jpeg)";
    document.body.style.backgroundSize = "cover";
  }
};
...
const getSpecificCity = async (city1, city2 = "chicago") => {
  try {
    ...
    const call = `https://api.openweathermap.org/data/2.5/weather?q=${
      city1 || city2
    ...
    const timeAndDateRelated = await getTimeFromCityName(
      city1,
      (city2 = "chicago")
    );
...
const getCurrentWeather = async () => {
  try {
    ...
    alert("Try a valid location...", error);
  }
};
